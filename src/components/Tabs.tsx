...
export default function Tabs(props: { tabs: Tab[] }) {
    ...
    return (
        <div>
            <div className="bg-tabs-background-light flex rounded-full p-2 gap-2 border border-zinc-200 mb-5">
                {props.tabs.map(tab => (
                    <button
                        key={tab.label}
                        onClick={() => setSelectedTabLabel(tab.label)}
                        className={`py-4 flex-1 rounded-full ${tab.label === selectedTabLabel ? 'shadow font-bold bg-white' : ''}`}
                    >
                        {tab.label}
                    </button>
                ))}
